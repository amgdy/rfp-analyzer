...
def generate_score_report_v2(results: dict) -> str:
    ...
    # Build the report using list for O(n) instead of O(n²) string concatenation
    parts = []
    parts.append(f"""# 📊 RFP Evaluation Report (Multi-Agent)
...
""")
    ...
    for c in extracted.get("criteria", []):
        parts.append(f"| {c.get('criterion_id', '')} | {c.get('name', '')} | {c.get('category', '')} | {c.get('weight', 0):.1f}% |\n")
...
""")
    ...
    parts.append(f"| **TOTAL** | **100%** | - | **{total_weighted:.2f}** |\n")
...
""")
    ...
    for cs in criterion_scores:
        ...
        parts.append(f"""### {indicator} {criterion_id}. {criterion_name}
...
""")
    if overall_strengths:
        ...
        parts.append("- No specific strengths identified\n")
...
""")
    if overall_weaknesses:
        ...
        parts.append("- No significant weaknesses identified\n")
...
""")
    if recommendations:
        ...
        parts.append("1. No specific recommendations at this time\n")
...
*Report generated by RFP Analyzer (Multi-Agent) - Powered by Azure Content Understanding & Microsoft Agent Framework*
""")
...
def generate_score_report(results: dict) -> str:
    ...
    # Build the report using list for O(n) instead of O(n²) string concatenation
    parts = []
    parts.append(f"""# 📊 Requirement Scores Report
...
""")
    ...
    for req in requirements:
        ...
        parts.append(f"| **{req_id}. {req_name}** | {req_text} | {eval_stage} | {target_val} | {response_val} | {max_score} | **{score:.2f}** | {weight:.0f}% | **{weighted_score:.2f}** |\n")
    ...
    parts.append(f"| **TOTAL** | | | | | **{total_max}** | **{total_score:.2f}** | **{total_weight:.0f}%** | **{total_weighted:.2f}** |\n")
...
""")
    ...
    for req in requirements:
        ...
        parts.append(f"""### {indicator} {req_id}. {req_name}
