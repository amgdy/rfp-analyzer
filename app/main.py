...
logger.info(
    "Optional features - PDF: %s, Markdown: %s, Plotly: %s",
    PDF_AVAILABLE,
    MARKDOWN_AVAILABLE,
    PLOTLY_AVAILABLE,
)
...
def get_project_dependency_versions() -> List[tuple[str, str | None]]:
    ...
    try:
        ...
        logger.warning(
            "Unable to read project dependencies from %s: %s", pyproject_path, exc
        )
        return []
...
def generate_pdf_from_markdown(
    markdown_content: str, title: str = "RFP Score Report"
) -> bytes | None:
...
async def process_document(
    ...
    logger.info(
        "Starting document processing: %s using %s", filename, extraction_service.value
    )
    ...
    logger.info(
        "Document processed: %s (%.2fs, %d chars)", filename, duration, len(content)
    )
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        dependency_versions = get_project_dependency_versions()
        if dependency_versions:
            with st.expander("📦 Python Packages"):
                for package_name, package_version in dependency_versions:
                    if package_version:
...
def render_landing_page():
    ...
    with col2:
        ...
        st.markdown("")
        st.caption(
            "Tip: Have your RFP document and vendor proposals ready before starting."
        )
...
def render_step1():
    ...
    st.markdown(
        "Upload the RFP document and vendor proposal files to begin the analysis."
    )
    ...
    with col1:
        ...
        if rfp_file is not None:
            ...
            if (
                st.session_state.rfp_file is None
                or st.session_state.rfp_file.get("name") != rfp_file.name
            ):
                logger.info(
                    "RFP file uploaded: %s (%.1f KB)",
                    rfp_file.name,
                    rfp_file.size / 1024,
                )
    ...
    with col2:
        ...
        if proposal_files:
            ...
            # Log new uploads
            current_names = (
                {p.get("name") for p in st.session_state.proposal_files}
            ...
            if current_names != new_names:
                logger.info(
                    "Proposal files uploaded: %d files - %s",
                    len(proposal_files),
                    ", ".join(f.name for f in proposal_files),
                )

            st.session_state.proposal_files = [
                {"bytes": f.getvalue(), "name": f.name} for f in proposal_files
            ]
    ...
    st.markdown(
        "Add additional evaluation criteria that will be used alongside criteria extracted from the RFP."
    )
    ...
    # Proceed button
    can_proceed = (
        st.session_state.rfp_file is not None
        and len(st.session_state.proposal_files) > 0
    )
    ...
    if can_proceed:
        if st.button(
            ...
            logger.info(
                "User proceeding to Step 2 - RFP: %s, Proposals: %d",
        ...
        st.info(
            "📌 Please upload an RFP file and at least one vendor proposal to continue."
        )
...
def render_step2():
    ...
    st.markdown(
        "Extract content from uploaded documents and configure evaluation settings."
    )
        ...
        if st.session_state.extraction_queue:
            ...
            with st.expander("📊 Extraction Summary", expanded=False):
                ...
                with col3:
                    st.metric(
                        "Avg per Doc",
                        format_duration(queue.get_average_item_duration()),
                    )

                for item in queue.items:
                    if item.duration:
                        st.markdown(
                            f"• **{item.name}**: `{format_duration(item.duration)}`"
                        )

        # Show extracted content previews
        with st.expander("📄 RFP Content Preview", expanded=False):
            st.markdown(
                st.session_state.rfp_content[:2000] + "..."
                if len(st.session_state.rfp_content) > 2000
                else st.session_state.rfp_content
            )
...
def run_extraction_pipeline():
    ...
    logger.info(
        "====== EXTRACTION PIPELINE STARTED (Service: %s) ======",
        extraction_service.value,
    )
    ...
    logger.info(
        "Queued RFP for extraction: %s (request_id: %s)",
        rfp_file["name"],
        rfp_request_id,
    )
    ...
    for i, proposal_file in enumerate(st.session_state.proposal_files):
        ...
        logger.info(
            "Queued proposal for extraction: %s (request_id: %s)",
            proposal_file["name"],
            proposal_request_id,
        )
    ...
    try:
        ...
        async def process_all_documents():
            ...
            for file_data in all_files:
                task = process_document(
                    file_data["bytes"], file_data["name"], extraction_service
                )
                tasks.append(task)
        ...
        def render_status():
            ...
            with status_placeholder.container():
                ...
                st.markdown(
                    f"**⏱️ Elapsed: `{format_duration(elapsed)}`** | Processing {total_files} documents in parallel..."
                )
                ...
                for item in extraction_queue.items:
                    ...
                    elapsed_time = (
                        format_duration(item.get_elapsed_time())
                        if item.start_time
                        else "-"
                    )
        ...
        logger.info(
            "====== EXTRACTION PIPELINE COMPLETED in %.2fs ======", total_duration
        )

        # Show completion message
        st.success(
            f"✅ **All {total_files} documents extracted in {format_duration(total_duration)}!**"
        )
...
def render_step3():
    ...
    st.markdown(
        "Evaluating vendor proposals against RFP requirements and generating comparison."
    )
    ...
    if st.session_state.evaluation_results and st.session_state.comparison_results:
        ...
        if st.button(
            ...
            logger.info(
                "User starting evaluation - Mode: individual, Effort: %s, Proposals: %d",
                st.session_state.reasoning_effort,
                len(st.session_state.proposal_files),
            )
            st.session_state.is_processing = True
...
def run_evaluation_pipeline():
    ...
    logger.info(
        "====== EVALUATION PIPELINE STARTED (Effort: %s) ======", reasoning_effort
    )
    pipeline_start = time.time()
    ...
    for i, proposal_file in enumerate(proposal_files):
        ...
        scoring_queue.add_item(
            ...
            metadata={"filename": proposal_file["name"], "request_id": request_id},
        )
        logger.info(
            "Queued proposal for scoring: %s (request_id: %s)",
            proposal_file["name"],
            request_id,
        )
    ...
    def render_status():
        ...
        with status_placeholder.container():
            ...
            for item in scoring_queue.items:
                icon = item.get_status_icon()
                elapsed_time = (
                    format_duration(item.get_elapsed_time()) if item.start_time else "-"
                )
                ...
                if item.status == QueueItemStatus.COMPLETED:
                    ...
                    if (
                        ...
                        and "total_score" in item.result
                    ):
    ...
    try:
        ...
        async def evaluate_all_proposals():
            ...
            for i, proposal_file in enumerate(proposal_files):
                ...
                proposal_content = st.session_state.proposal_contents.get(
                    proposal_name, ""
                )
        ...
        if len(evaluation_results) > 1:
            ...
            try:
                ...
                comparison_results = asyncio.run(
                    comparison_agent.compare_evaluations(
                        evaluation_results, rfp_title, reasoning_effort=reasoning_effort
                    )
                )
            ...
            if evaluation_results:
                st.session_state.comparison_results = {
                    "rfp_title": evaluation_results[0].get(
                    ...
                    "vendor_rankings": [
                        {
                            "rank": 1,
                            "vendor_name": evaluation_results[0].get(
                                "supplier_name", "Unknown"
                            ),
                            "total_score": evaluation_results[0].get("total_score", 0),
                            "grade": evaluation_results[0].get("grade", "N/A"),
                            "key_strengths": evaluation_results[0].get(
                                "overall_strengths", []
                            )[:3],
                            "key_concerns": evaluation_results[0].get(
                                "overall_weaknesses", []
                            )[:3],
                            "recommendation": evaluation_results[0].get(
                                "recommendation", ""
                            ),
                        }
                    ],
                    "selection_recommendation": evaluation_results[0].get(
                        "recommendation", ""
                    ),
                    "comparison_insights": [],
                }
        ...
        logger.info(
            "====== EVALUATION PIPELINE COMPLETED in %.2fs ======", total_duration
        )
...
def render_metrics_dashboard(comparison: dict, evaluations: list):
    ...
    st.markdown(
        "Visual comparison of vendor performance across all evaluation criteria."
    )
    ...
    if not PLOTLY_AVAILABLE:
        st.warning(
            "📊 Plotly is not installed. Install it with `pip install plotly` for interactive charts."
        )
    ...
    st.markdown(
        "Each chart shows how vendor scores compare for a specific evaluation criterion. "
    ...
    for i in range(0, num_criteria, cols_per_row):
        ...
        for j in range(cols_per_row):
            ...
            criterion_name = criterion.get(
                "criterion_name", f"Criterion {criterion_idx + 1}"
            )
            criterion_weight = criterion.get("weight", 0)

            with cols[j]:
                _render_criterion_bar_chart(
                    evaluations, criterion_idx, criterion_name, criterion_weight
                )
...
def _render_overall_comparison_bar(evaluations: list):
    ...
    sorted_data = sorted(
        zip(vendor_names, total_scores, grades), key=lambda x: x[1], reverse=True
    )
    vendor_names, total_scores, grades = (
...
def _render_criterion_bar_chart(
    evaluations: list, criterion_idx: int, criterion_name: str, weight: float
):
...
def _render_criterion_recommendations(comparison: dict, evaluations: list):
    ...
    if not criterion_comparisons:
        ...
        for criterion_idx, criterion in enumerate(criteria):
            ...
            if best_vendor:
                with st.expander(f"**{criterion_name}** - Recommended: {best_vendor}"):
                    st.markdown(
                        f"**Best Performer:** {best_vendor} (Score: {best_score:.1f}/100)"
                    )
        ...
        for cc in criterion_comparisons:
            ...
            with st.expander(
                f"**{criterion_name}** (Weight: {weight:.1f}%) - Recommended: {best_vendor}"
            ):
                col1, col2, col3 = st.columns(3)
                with col1:
...
def _render_basic_metrics_dashboard(comparison: dict, evaluations: list):
    ...
    for eval_result in sorted(
        evaluations, key=lambda x: x.get("total_score", 0), reverse=True
    ):
        vendor_name = eval_result.get("supplier_name", "Unknown")
        total_score = eval_result.get("total_score", 0)
        grade = eval_result.get("grade", "N/A")
...
def render_comparison_overview(comparison: dict, evaluations: list):
    ...
    if criterion_comparisons:
        ...
        for cc in criterion_comparisons:
            with st.expander(
                f"**{cc.get('criterion_name', 'Unknown')}** (Weight: {cc.get('weight', 0):.1f}%)"
            ):
                col1, col2 = st.columns(2)
                with col1:
...
def render_individual_reports(evaluations: list):
    ...
    for i, eval_result in enumerate(evaluations):
        ...
        with st.expander(
            f"**{vendor_name}** - Score: {total_score:.1f} ({grade})", expanded=i == 0
        ):
            # Score summary
            col1, col2, col3 = st.columns(3)
            with col1:
            ...
            for cs in criterion_scores:
                ...
                with st.container():
                    st.markdown(
                        f"{bar_color} **{criterion_name}**: {score_pct:.1f}/100 (weighted: {weighted_score:.2f})"
                    )
                    if justification:
...
def render_detailed_scores(evaluations: list):
    ...
    if evaluations[0].get("criterion_scores"):
        criteria = [
            cs.get("criterion_name", cs.get("criterion_id", f"C-{i}"))
...
def render_export_options(comparison: dict, evaluations: list):
    ...
    st.markdown(
        "Complete report including comparison, rankings, and all vendor details."
    )
    ...
    if comparison and evaluations:
        ...
        if full_report:
            ...
            st.caption(
                "Word export not available. Install python-docx with: pip install python-docx"
            )
    ...
    for i, eval_result in enumerate(evaluations):
        ...
        with vendor_cols[col_idx]:
            vendor_name = eval_result.get("supplier_name", f"Vendor_{i + 1}").replace(
                " ", "_"
            )
    ...
    with col3:
        if PDF_AVAILABLE and MARKDOWN_AVAILABLE:
            ...
            pdf_data = generate_pdf_from_markdown(
                report_md, title=f"RFP Score Report - {supplier_name}"
            )
            if pdf_data:
                st.download_button(
                    label="📑 Download PDF",
...
def render_results_v2(results: dict):
    ...
    with col3:
        if PDF_AVAILABLE and MARKDOWN_AVAILABLE:
            pdf_data = generate_pdf_from_markdown(
                report_md, title=f"RFP Score Report V2 - {supplier_name}"
            )
            if pdf_data:
                st.download_button(
                    label="📑 Download PDF",
                ...
                st.button(
                    "📑 PDF Generation Failed", disabled=True, use_container_width=True
                )
        else:
